...
public class ChineseUtil {
	...
	private static ChineseUtil chineseSpelling = new ChineseUtil();
	...
	public String getSelling(String chs) {
		...
		for (int i = 0; i < chs.length(); i++) {
			...
			int m=0;
			try {
				m = key.getBytes("UTF-8").length;
			} catch (UnsupportedEncodingException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
			// android上一个汉字是3byte，一个字母是1byte
	...
	public static void main(String[] args) {
		ChineseUtil finder = ChineseUtil.getInstance();
		System.out.println(finder.getSelling("英/文字符Eng"));
	}
